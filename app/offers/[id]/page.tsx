...
import { MarginDialog } from "@/components/offers/MarginDialog";
import { DiscountDialog } from "@/components/offers/DiscountDialog";
...
export default function OfferEditorPage({
  ...
  const [activeDialog, setActiveDialog] = useState<"margins" | "discount" | null>(
    null
  );
  ...
  async function addGroup() {
    const res = await fetch(`/api/offers/${params.id}/groups`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title: "Neue Gruppe" })
    });
    if (!res.ok) return;
    void load();
  }
  ...
  async function addItem(groupId: string) {
    const res = await fetch(
      `/api/offers/${params.id}/groups/${groupId}/items`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    <div className="min-h-screen bg-zinc-50">
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Menu as="div" className="relative">
              <Menu.Button className="px-4 py-2 text-sm rounded-lg hover:bg-zinc-50">
                Aktionen
              </Menu.Button>
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-8 space-y-6">
            ...
            {customer && (
              <div className="rounded-xl border bg-white p-4 space-y-4">
                <h2 className="text-base font-medium">Anschrift</h2>
                ...
                <div className="space-y-1">
                  ...
                  {customer.billing_address_extra && (
                    <div>{customer.billing_address_extra}</div>
                  )}
                  <div>
          ...
          <div className="col-span-4">
            <SummaryBox
              ...
              onAdjustMargins={() => setActiveDialog("margins")}
              onAddDiscount={() => setActiveDialog("discount")}
              taxRate={tax_rate}
              showLaborTax={show_vat_for_labor}
              onToggleLaborTax={() =>
                save({ show_vat_for_labor: !show_vat_for_labor })
              }
            />
          </div>
        </div>
      ...
      {activeDialog === "margins" && (
        <MarginDialog
          ...
          onApply={async (margins) => {
            // TODO: Margen auf alle Positionen anwenden
      ...
      {activeDialog === "discount" && (
        <DiscountDialog
          onApply={async (values) => {
            await save({
              discount_percent: values.percent,
              discount_days: values.days
            });
            setActiveDialog(null);
          }}
          onClose={() => setActiveDialog(null)}
